...
if [ ! -f "$RESTORE_DIR/xwiki/db.sql.gz" ]; then
  echo "ERROR: $RESTORE_DIR/xwiki/db.sql.gz not found"
  exit 1
fi
...
docker compose up -d db --wait || exit 1
gunzip < $RESTORE_DIR/xwiki/db.sql.gz | docker compose exec -T db mysql -u root -p"$MYSQL_ROOT_PASSWORD" || exit 1
docker compose up -d
